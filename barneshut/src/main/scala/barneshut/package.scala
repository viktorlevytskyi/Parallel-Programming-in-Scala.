...
package object barneshut {
  ...
  case class Fork(
    ...
    val centerX: Float = nw.centerX + ne.size / 2
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      this(((b.x - boundaries.minX) / sectorSize).toInt min (sectorPrecision - 1) max 0,
        ((b.y - boundaries.minY) / sectorSize).toInt min (sectorPrecision - 1) max 0) += b
      this
    }
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      ...
      this.matrix.foreach(_.foreach(result += _))
      that.matrix.foreach(_.foreach(result += _))
      result
    }
